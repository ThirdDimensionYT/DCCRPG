...
export default function DiceRoller() {
  ...
  return <aside className={`dice-drawer ${open ? 'open' : ''}`} aria-label="Dice roller">
    {open ? <section className="dice-drawer-panel">
      ...
      <div className={`animated-die ${rolling ? 'rolling' : ''}`} aria-live="polite"><span>{rolling ? '?' : latest?.total ?? '20'}</span><small>{rolling ? 'Rolling…' : latest?.notation ?? 'Ready'}</small></div>
    ...
    <button type="button" className={`dice-fab ${rolling ? 'rolling' : ''}`} onClick={() => setOpen((current) => !current)} aria-expanded={open}><span>⚄</span><strong>{latest ? latest.total : 'Roll dice'}</strong></button>
  </aside>
}
