...
export default function CharacterSheetEditor({ character, saving, isAdmin, onCancel, onSave }: {
  ...
  return <form key={character.id} onSubmit={(event) => void submit(event)}>
    ...
    <section className="sheet-editor-section" hidden={tab !== 'core'}><h3>Identity & status</h3>
      <div className="form-grid three"><label>Name<input name="name" required maxLength={80} defaultValue={character.name} /></label><label>Race<input name="race" required maxLength={80} defaultValue={character.race} /></label><label>Gender / pronouns<input name="genderPronouns" maxLength={80} defaultValue={character.gender_pronouns} /></label><label>Level<input name="level" type="number" min="1" max="250" defaultValue={character.level} /></label><label>Crawler number<input name="crawlerNumber" type="number" min="1" max="12900000" defaultValue={character.crawler_number} /></label><label>Class<input name="className" required maxLength={80} defaultValue={character.class_name} /></label></div>
      <div className="health-editor"><div><strong>Health</strong><small>{10-healthLost}/10 segments</small></div><input aria-label="Health segments lost" type="range" min="0" max="10" value={healthLost} onChange={(event) => setHealthLost(Number(event.target.value))} /><div className="health-segments">{Array.from({length:10},(_,index) => <span className={index >= 10-healthLost ? 'lost' : ''} key={index}>{(index+1)*10}%</span>)}</div></div>
